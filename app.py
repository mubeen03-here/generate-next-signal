...
@st.cache_resource
def get_db_engine():
    """Optimized Neon Postgres Connection for Serverless environments."""
    db_url = st.secrets["DATABASE_URL"]
    if "?sslmode=" not in db_url:
        db_url += "?sslmode=require"
    
    # Reduced pool limits to prevent connection exhaustion on Streamlit Cloud
...
def cancel_all_pending_signals():
    """Cancels all active pending signals."""
    with engine.begin() as conn:
        conn.execute(text("""
            UPDATE signals_v2 
...
def run_auto_timeout_and_updates(symbol, current_price):
    """Cancels >24h stale signals and triggers TP/SL hits."""
    with engine.begin() as conn:
        conn.execute(text("""
            UPDATE signals_v2 
            SET status = 'CLOSED', result = 'CANCELLED' 
...
def is_macro_news_blocked():
    """Blocks trading during high impact USD news."""
    try:
...
def generate_svg_chart(df, width=900, height=350):
    ...
    padding_top, padding_bottom, padding_left, padding_right = 25, 35, 70, 25
    plot_width = width - padding_left - padding_right
    plot_height = height - padding_top - padding_bottom
    
    def get_x(i): return padding_left + (i / (max(len(close_vals) - 1, 1))) * plot_width
    def get_y(val): return padding_top + plot_height - ((val - min_val) / val_range) * plot_height
    ...
    <svg viewBox="0 0 {width} {height}" width="100%" height="100%" xmlns="http://www.w3.org/2000/svg" style="background-color: #0E1117; border-radius: 8px; font-family: system-ui, sans-serif;">
        ...
        <text x="{get_x(0)}" y="{height - 12}" fill="#808495" font-size="11" text-anchor="middle">{dates[0] if dates else ''}</text>
        <text x="{get_x(len(dates)//2)}" y="{height - 12}" fill="#808495" font-size="11" text-anchor="middle">{dates[len(dates)//2] if dates else ''}</text>
        <text x="{get_x(len(dates)-1)}" y="{height - 12}" fill="#808495" font-size="11" text-anchor="middle">{dates[-1] if dates else ''}</text>
...
def generate_signals_engine(selected_symbol, timeframe="15m"):
    ...
    try:
        ...
        # REQUIRED: Copying df to prevent CachedObjectMutationWarning
        df = raw_df.copy()
        
        # Safe MultiIndex resolution
        ...
        return {
            ...
            "plot_df": df[['Close', 'VWAP']].tail(50).copy()
        }
...
if engine_output:
    ...
    st.error("⚠️ Yahoo Finance API rate-limited. Feed delayed.")
...
try:
    ...
    if active_signals:
        df_signals = pd.DataFrame(active_signals, columns=["Time", "Type", "Asset", "Entry", "TP", "SL", "Status", "Result"])
        st.dataframe(df_signals, use_container_width=True)
    else:
        st.info("No active signals recorded yet. Waiting for sweeps...")
except Exception as e:
